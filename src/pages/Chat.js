...
const Chat = () => {
  ...
  const selectUser=async (user)=>{
    ...
    const id=user1>user2?`${user1}.${user2}.${user.ad.adId}`:`${user2}.${user1}.${user.ad.adId}`;
    const msgsRef=collection(db,'messages',id,"chat");
    const q=query(msgsRef,orderBy('createdAt',"asc"));

     const unsub=onSnapshot(q,querySnapshot=>{
      let inmsgs=[];
      querySnapshot.forEach(doc=>inmsgs.push(doc.data()));
      setMsgs(inmsgs);
    })
  ...
  return (
    <div className="row g-0">
      ...
      >
        {users.map((user, i) => (
          <User key={i} user={user} selectUser={selectUser} chat={chat} online={online} user1={user1}/>
        ))}
      </div>
